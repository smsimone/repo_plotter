...
flags.DEFINE_string("repository", None,
                    "The repository to execute this script on")
flags.DEFINE_string(
    "input_file",
    None,
    "Specifies a custom json file to feed the script. It must has been generated during an old execution",
)
flags.DEFINE_string("branch", None, "Select a custom branch of the repository")
flags.DEFINE_bool(
    "no_preprocessing",
    False,
...
def generate_repo_history(temporary_dir: str) -> RepoHistory:
    ...
    if not FLAGS.offline:
        ...
        try:
            ...
            subprocess.call(
                ["git", "clone", repository, temporary_dir, "--quiet"])
        except:
        ...
        if not os.path.exists(temporary_dir):
            print(
                "You can't use --offline flag on a non-existent --dir directory"
            )
            exit(-1)
...
def main(args):
    ...
    if write_output:
        with open(f"{output_folder}/repo_history_preprocessed.json",
                  "w+") as f:
            json.dump(repo_history.as_map(), f)
